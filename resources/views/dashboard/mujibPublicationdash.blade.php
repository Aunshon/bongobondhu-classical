...
<table class="table table-info">
  ...
  <tbody>
        @foreach ($mujibPublicationdash as $item)
            <tr>
                <td>{{$item->title}}</td>
                <td><textarea cols="30" rows="10" readonly>{{$item->dis}}</textarea></td>
                <td><img width="200" src="{{asset('frontEnd/uploads/mujib_Publication')}}/{{$item->photo}}" alt="No Photo" srcset=""></td>
                <td><a class="btn btn-danger" href="{{url('/delete_mujibPublicationdash/delete')}}/{{$item->id}}">Delete</a></td>
