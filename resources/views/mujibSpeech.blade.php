    ...
    @foreach ($alldata as $item)
        @if ($loop->first)
            <div class="history_content hcOne aunShow">
                ...
                <span class="changeBtn_one aunArro arroTOChange" data-imgUrl="frontEnd/uploads/mujib_Speech/{{$item->photo}}">
                    &#8595;
                </span>
            </div>
        @else
            <div class="history_content hcTwo d-none">
                ...
                <span class="changeBtn aunArro" data-imgUrl="{{asset('frontEnd/uploads/mujib_Speech')}}/{{$item->photo}}">
                    &#8595;
                </span>
            </div>
        @endif
  ...
  <div class="col-lg-8 order-0">
      <div class="history_main_image">
         <div class="history_img">
                ...
                @foreach ($alldata as $item)
                    @if ($loop->first)
                    <img class="w-100 timeLine-1 tm-1 aunShow aunImg" src="{{asset('frontEnd/uploads/mujib_Speech')}}/{{$item->photo}}" alt="Bongobondhu">
                    @else
                    <img class="w-100 timeLine-1 tm-2 d-none" src="{{asset('frontEnd/uploads/mujib_Speech')}}/{{$item->photo}}" alt="Bongobondhu">
                    @endif
                @endforeach
